...
- **2026-07-05 (FEAT-016)** — Added `IGitHubService.GetFileContentAsync` (read) alongside the existing `UpsertFileAsync` (write), completing the bidirectional GitHub sync surface. Added `IProjectDocSyncService.PullDocsFromRepoAsync` cross-module contract (in `DevHub.Contracts`) implemented by `ProjectDocsService`. `CheckpointSignalsService` fires a background `PullDocsFromRepoAsync` on work-item `Completed`; the call uses a fresh DI scope and `CancellationToken.None` because the HTTP request scope is disposed by the time the task runs. `ProjectDocsService` gained static helpers `AssembleDocMarkdown` (write path) and `ParseDocMarkdown` (read path), both `internal static` for direct unit testing.
