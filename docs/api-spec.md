...
- **2026-07-05 (FEAT-016)** — `PUT /api/projects/{id}/docs/{key}` response gained `repoSynced: bool | null` at the lock transition (first call that fills all required sections). `true` = all 7 doc files successfully pushed to the project's GitHub repo; `false` = push attempted but failed; `null` = no repo configured. No new public endpoints. Work-item `Completed` signals now trigger a background pull from the project repo into the DB — no executor contract change required.

- **2026-07-11 (FEAT-017)** — New checkpoint contract `confirm_mockup` registered with `allowedOutcomes: ["approve", "reject"]`. `OrchestratorExecutorClient` maps `checkpointKey="confirm_mockup"` → orchestrator signal `name="mockup-approved"` via a `ResolveSignalName` helper (all other checkpoint keys remain identity-mapped). `feedback` carried in the signal `payload` on reject.
