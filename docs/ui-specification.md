...
- **2026-07-11 (FEAT-017)** — Lifecycle review page gained `MockupReviewPanel`, rendered when `currentCheckpointKey === 'confirm_mockup'`. Displays `mockupHtml` in a sandboxed `<iframe srcdoc>` with `mockupDescription` caption and `currentTask` context. Approve emits `{ outcome: 'approve' }`; reject reveals a feedback textarea and emits `{ outcome: 'reject', payload: { feedback } }` (submit disabled while empty). `TaskListReviewPanel` gained a sky-100/sky-700 `Mockup` badge on tasks with `kind="mockup"`; all other kinds show no badge.
