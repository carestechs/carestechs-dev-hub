...
## Overview
Install EF Core + Npgsql + naming-convention package in every module project, publish `BaseEntity`/`ISoftDeletable`/`TimestampingInterceptor` from `Portfolio.Contracts`, and scaffold an empty `<Module>DbContext` per module that applies the snake_case convention and the timestamping interceptor.

## Implementation Steps

### Step 1: Add EF + Npgsql packages to every module
**File:** `src/Portfolio.Modules.<Module>/Portfolio.Modules.<Module>.csproj`
**Action:** Modify
...
**File:** `src/Portfolio.Contracts/Persistence/BaseEntity.cs`
**Action:** Create
```csharp
...
**File:** `src/Portfolio.Contracts/Persistence/ISoftDeletable.cs`
**Action:** Create
```csharp
...
**File:** `src/Portfolio.Contracts/Persistence/TimestampingInterceptor.cs`
**Action:** Create
...
**File:** `src/Portfolio.Modules.<Module>/<Module>DbContext.cs` (×6)
**Action:** Create
```csharp
...
**File:** `src/Portfolio.Modules.<Module>/<Module>ModuleExtensions.cs` (×6)
**Action:** Create
```csharp
...
```
Register `TimestampingInterceptor` as a singleton in `Portfolio.Api/Program.cs` (T-004).
...
| File | Action | Summary |
|------|--------|---------|
| `src/Portfolio.Modules.<Module>/*.csproj` (×6) | Modify | Add EF Core + Npgsql + EFCore.NamingConventions |
| `src/Portfolio.Contracts/Persistence/BaseEntity.cs` | Create | Shared base entity |
| `src/Portfolio.Contracts/Persistence/ISoftDeletable.cs` | Create | Soft-delete marker interface |
| `src/Portfolio.Contracts/Persistence/TimestampingInterceptor.cs` | Create | Auto-populates `CreatedAt`/`UpdatedAt` |
| `src/Portfolio.Modules.<Module>/<Module>DbContext.cs` (×6) | Create | Empty DbContext with schema default |
| `src/Portfolio.Modules.<Module>/<Module>ModuleExtensions.cs` (×6) | Create | `Add<Module>Module` extension wiring the DbContext |
...
- [ ] `BaseEntity` and `ISoftDeletable` exist in `Portfolio.Contracts/Persistence/`.
