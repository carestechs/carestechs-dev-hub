...
- **Workflow:** standard
- **Complexity:** L
- **Rationale:** Profile rule "thin API host": no controllers, services, or business logic in `Portfolio.Api`. Centralized exception handler produces uniform problem-details across modules.

## Overview
Compose the API host: DI registration, JWT bearer, RFC 7807 exception handler, CORS, request logging, health-check registration, and the `Add<Module>Module()` / `Use<Module>Module()` invocations. Publish `DomainException` family from `Portfolio.Contracts`.

## Implementation Steps

### Step 1: Domain exception family
**File:** `src/Portfolio.Contracts/ApplicationErrors/DomainException.cs` (plus siblings)
**Action:** Create
...
**File:** `src/Portfolio.Api/Middleware/ProblemDetailsHandler.cs`
**Action:** Create
...
### Step 3: appsettings
**Files:** `src/Portfolio.Api/appsettings.json`, `appsettings.Development.json`
**Action:** Create
...
**File:** `src/Portfolio.Api/Options/JwtOptions.cs`, `CorsOptions.cs`, `OperatorSeedOptions.cs`
**Action:** Create
...
| File | Action | Summary |
|------|--------|---------|
| `src/Portfolio.Contracts/ApplicationErrors/*.cs` | Create | Domain exception family |
| `src/Portfolio.Api/Middleware/ProblemDetailsHandler.cs` | Create | RFC 7807 translator |
| `src/Portfolio.Api/appsettings.json`, `appsettings.Development.json` | Create | Config defaults |
| `src/Portfolio.Api/Options/*.cs` | Create | Strongly-typed, validated options |
| `src/Portfolio.Api/Program.cs` | Modify | Composition root |
| `src/Portfolio.Modules.<Module>/<Module>ModuleExtensions.cs` | Modify | `Add<Module>Module` extensions called by Program.cs |
...
- **Module registration order** — independent today, but if a module's `AddXModule` ever depends on services from another, the dependency must be made explicit through `Portfolio.Contracts`.

## Acceptance Verification
- [ ] `Program.cs` contains only DI registration and pipeline composition; `grep "class.*Controller" src/Portfolio.Api/` returns nothing.
