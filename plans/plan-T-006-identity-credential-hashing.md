...
**File:** `src/Portfolio.Modules.Identity/Portfolio.Modules.Identity.csproj`
**Action:** Modify
Add `<PackageReference Include="Konscious.Security.Cryptography.Argon2" Version="1.3.*" />`.

### Step 2: Entities
**Files:** `src/Portfolio.Modules.Identity/Entities/IdentityCredential.cs`, `RefreshToken.cs`, `Enums/CredentialProvider.cs`
**Action:** Create
...
**File:** `src/Portfolio.Modules.Identity/IdentityDbContext.cs`
**Action:** Modify
...
### Step 4: Migration
**Action:** Generate
`dotnet ef migrations add Initial --project src/Portfolio.Modules.Identity --startup-project src/Portfolio.Api`. Verify schema = `identity`, columns snake_case, no cross-module FK constraints (Workspace.Member.Id is referenced by ID only — no DB-level FK).

### Step 5: Password hasher
**Files:** `src/Portfolio.Modules.Identity/Services/IPasswordHasher.cs`, `Argon2PasswordHasher.cs`
**Action:** Create
```csharp
...
**File:** `src/Portfolio.Modules.Identity/Seeding/IdentitySeeder.cs`
**Action:** Create
`IHostedService`. On `StartAsync` (must run **after** WorkspaceSeeder):
1. `await db.Database.MigrateAsync()`.
2. Resolve seed operator member id via `IProjectMembershipQuery` or a temporary `IMemberLookup` helper published from `Portfolio.Contracts`. (Add a simple `IMemberLookup.FindByEmailAsync(email)` to Contracts; Workspace implements it.)
...
| File | Action | Summary |
|------|--------|---------|
| `src/Portfolio.Modules.Identity/Portfolio.Modules.Identity.csproj` | Modify | Add Argon2 package |
| `src/Portfolio.Modules.Identity/Entities/IdentityCredential.cs` | Create | Credential entity |
| `src/Portfolio.Modules.Identity/Entities/RefreshToken.cs` | Create | Refresh-token entity |
| `src/Portfolio.Modules.Identity/Entities/Enums/CredentialProvider.cs` | Create | Enum |
| `src/Portfolio.Modules.Identity/IdentityDbContext.cs` | Modify | DbSets + mappings |
| `src/Portfolio.Modules.Identity/Migrations/*` | Create | Initial migration |
| `src/Portfolio.Modules.Identity/Services/IPasswordHasher.cs` | Create | Interface |
| `src/Portfolio.Modules.Identity/Services/Argon2PasswordHasher.cs` | Create | Argon2id implementation |
| `src/Portfolio.Modules.Identity/Seeding/IdentitySeeder.cs` | Create | Seeds operator credential |
| `src/Portfolio.Contracts/Identity/IMemberLookup.cs` | Create | Cross-module ID resolution |
| `src/Portfolio.Modules.Workspace/Services/MemberLookup.cs` | Create | Workspace's implementation |
| `src/Portfolio.Modules.Identity/IdentityModuleExtensions.cs` | Modify | Register hasher + seeder |
...
- **Cross-module member lookup** — Identity must not load Workspace's `Member` entity directly. Use the `IMemberLookup` interface published in `Portfolio.Contracts`; Workspace implements it; this preserves the "ID-only cross-module reference" rule.
